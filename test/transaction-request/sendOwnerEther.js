...
contract("TransactionRequestCore", async (accounts) => {
  ...
  before(async () => {
    ...
    const funds = toBN(10).pow(toBN(18)); // 1 ETH
    const delay = 50;
    const payout = toBN(10).pow(toBN(17)); // 0.1 ETH
    const gasPrice = toBN(10).pow(toBN(6));
    ...
    expect(proxyBalance).to.bignumber.equal('0');
    ...
    expect(walletBalanceAfterExecution).to.bignumber.equals(initialWalletBalance.add(payout));

    remainingScheduledTransactionBalance = toBN(await getBalance(scheduledTransactionAddress));
    expect(remainingScheduledTransactionBalance).to.be.bignumber.greaterThan('0');
  });
  ...
  it("should fail to send remaining funds to the contract", async () => {
    ...
    expect(scheduledTransactionBalance).to.bignumber.equal(remainingScheduledTransactionBalance);
  });
  ...
  it("should fail to send remaining funds to the account when not owner", async () => {
    ...
    const scheduledTransactionBalance = await getBalance(scheduledTransactionAddress);
    expect(scheduledTransactionBalance).to.be.bignumber.equal(remainingScheduledTransactionBalance);
  });

  it("should fail to send remaining funds to null account", async () => {
    expect(proxy.sendOwnerEther('')).to.eventually.be.rejected;

    // FIXME: this should probably wait for the above to be executed.
    const scheduledTransactionBalance = await getBalance(scheduledTransactionAddress);
    expect(scheduledTransactionBalance).to.bignumber.equal(remainingScheduledTransactionBalance);
  });
  ...
  it("should succeed to send remaining funds to the account", async () => {
    ...
    const scheduledTransactionBalance = await getBalance(scheduledTransactionAddress);
    expect(scheduledTransactionBalance).to.bignumber.equal('0');

    const walletBalance = await getBalance(wallet);
    expect(walletBalance)
      .to.bignumber.equal(walletBalanceAfterExecution.add(remainingScheduledTransactionBalance));
  });
});
