...
contract("Timestamp scheduling", (accounts) => {
  ...
  it("should do timestamp scheduling using `schedule", async () => {
    ...
    expect(requestData.schedule.windowSize).to.bignumber.equal(toBN(55 * MINUTE));

    expect(toBN(requestData.txData.callGas)).to.bignumber.equal('1212121');
    ...
    expect(requestData.claimData.requiredDeposit).to.bignumber.equal(toBN(requiredDeposit));
  });
