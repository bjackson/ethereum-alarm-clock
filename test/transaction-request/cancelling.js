...
contract("Cancelling", async (accounts) => {
  ...
  it("tests CANNOT cancel during the execution window", async () => {
    ...
    expect(cancelAt).to.be.bignumber.above(toBN(await config.web3.eth.getBlockNumber()));
  ...
  it("tests CANNOT cancel if was called", async () => {
    ...
    const cancelFirst = requestData.schedule.windowStart.addn(10);
    const cancelSecond = requestData.schedule.windowStart.add(requestData.schedule.windowSize).addn(5);

    expect(executeAt).to.be.bignumber.above(toBN(await config.web3.eth.getBlockNumber()));
  ...
  it("tests CANNOT cancel if already cancelled", async () => {
    ...
    const cancelFirst = requestData.schedule.windowStart.sub(requestData.schedule.freezePeriod).subn(20);
    const cancelSecond = requestData.schedule.windowStart.sub(requestData.schedule.freezePeriod).subn(10);

    expect(cancelFirst).to.be.bignumber.above(toBN(await config.web3.eth.getBlockNumber()));
