...
contract("Block claiming", async (accounts) => {
  ...
  it("should execute a claimed call after block reserve window", async () => {
    ...
    // );
    const advanceTo = requestData.schedule.windowStart.add(requestData.schedule.reservedWindowSize);
    // eslint-disable-next-line no-console
    console.warn(`Waiting for block: ${await config.web3.eth.getBlockNumber()} -> ${advanceTo}`);
    await time.advanceBlockTo(advanceTo);
  ...
  it("should determine bounty amount with modifier", async () => {
    ...
    const addTo = toBN(Math.floor((requestData.schedule.claimWindowSize.toNumber() * 2) / 3));
    ...
    const expectedPaymentModifier = toBN(Math.floor((100 * 2) / 3));

    expect(requestData.claimData.paymentModifier.toString()).to.equal('0');
    ...
    expect(requestData.claimData.paymentModifier.subn(2)).to.bignumber.equal(expectedPaymentModifier);
  });
  ...
  it("CANNOT claim with insufficient claim deposit", async () => {
    ...
    expect(firstClaimBlock).to.be.bignumber.above(toBN(await config.web3.eth.getBlockNumber()));
