...
contract("TransactionRequestCore proxy function", (accounts) => {
  ...
  it("only allows calling proxy after execution window and by the scheduling account", async () => {
    ...
    const windowStart = curBlockNum.addn(38).addn(10).addn(5);
    ...
    const duringExecutionWindow = (
      requestData.schedule.windowStart.add(requestData.schedule.windowSize)
    ).subn(2);
  ...
  it("does some fancy stuff with proxy", async () => {
    ...
    const requestData = await RequestData.from(txRequest);
    const executeAt = requestData.schedule.windowStart.addn(1);
    ...
    await waitUntilBlock(0, afterExecutionWindow.toNumber());
