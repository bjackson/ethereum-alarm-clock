...
contract("Tests execution gas requirements", async (accounts) => {
  ...
  beforeEach(async () => {
    ...
    await txRequest.initialize(
      ...
      web3.utils.fromAscii("here-I-am!!-the-call-data"),
  ...
  it("test direct execution rejected with insufficient gas", async () => {
    ...
    const minCallGas = requestData.txData.callGas
      .addn((await requestLib.EXECUTION_GAS_OVERHEAD()).toNumber());
  ...
  it("test direct execution accepted with minimum gas", async () => {
    ...
    await waitUntilBlock(0, requestData.schedule.windowStart.toNumber());

    const minCallGas = requestData.txData.callGas.addn((await requestLib.EXECUTION_GAS_OVERHEAD()).toNumber());
