...
contract("Schedule to execution flow", (accounts) => {
  ...
  it("should schedule a transaction", async () => {
    ...
    windowStart = toBN(curBlockNum + 20);
  ...
  it("verifies the txRequest data", async () => {
    ...
    expect(requestData.txData.callGas.toString()).to.equal('1212121');

    expect(requestData.txData.callValue.toString()).to.equal('123454321');

    expect(requestData.schedule.windowStart).to.bignumber.eq(windowStart);

    expect(requestData.txData.gasPrice).to.bignumber.equal(parseInt(gasPrice, 10).toString());

    expect(requestData.paymentData.fee).to.bignumber.equal('98765');

    expect(requestData.paymentData.bounty).to.bignumber.equal('80008');
  });
